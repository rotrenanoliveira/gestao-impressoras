...
export class InMemoryWorkstationsRepository implements WorkstationsRepository {
  ...
  async save(workstation: Workstation): Promise<void> {
    const workstationIndex = this.items.findIndex((item) => item.id === workstation.id)

    this.items[workstationIndex] = workstation
  }

  async delete(workstationId: string): Promise<void> {
    const workstationIndex = this.items.findIndex((item) => item.id.toString() === workstationId)
