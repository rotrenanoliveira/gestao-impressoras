...
export class InMemoryMobileDevicesRepository implements MobileDevicesRepository {
  ...
  async save(mobileDevice: MobileDevice): Promise<void> {
    const mobileDeviceIndex = this.items.findIndex((item) => item.id === mobileDevice.id)

    this.items[mobileDeviceIndex] = mobileDevice
  }

  async delete(mobileDeviceId: string): Promise<void> {
    const mobileDeviceIndex = this.items.findIndex((item) => item.id.toString() === mobileDeviceId)
