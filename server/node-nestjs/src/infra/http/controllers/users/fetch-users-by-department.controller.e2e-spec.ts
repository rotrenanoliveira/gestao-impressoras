...
describe('Fetch users by department (E2E)', () => {
  ...
  test('[GET] /users/department/:departmentId', async () => {
    ...
    const otherDepartment = await departmentFactory.makePrismaDepartment()
    ...
    const response = await request(app.getHttpServer()).get(`/users/department/${departmentId}`).send()
