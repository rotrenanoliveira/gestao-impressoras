...
describe('Inactive user license (E2E)', () => {
  ...
  test('[PATCH] /user-licenses/:id/inactive', async () => {
    ...
    const response = await request(app.getHttpServer()).patch(`/user-licenses/${userLicenseId}/inactive`).send()
    ...
    expect(userLicenseOnDatabase.status).toEqual('INACTIVE')
  })
})
