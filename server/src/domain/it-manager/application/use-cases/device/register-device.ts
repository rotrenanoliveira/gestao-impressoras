...
interface RegisterDeviceUseCaseProps {
...
type RegisterDeviceUseCaseResponse = Either<ResourceNotFoundError, { device: Device<DeviceProps> }>
...
export class RegisterDeviceUseCase implements UseCase {
  ...
  async execute({
    ...
    purchaseDate,
    warrantyEndDate,
  }: RegisterDeviceUseCaseProps): Promise<RegisterDeviceUseCaseResponse> {
    if (contractId) {
